...
class Sfb2XConverter {
	...
	protected $titles = array(1 => 'h2', 'h3', 'h4', 'h5', 'h6');
	...
	public function __construct($file, $imgDir = 'img/') {
		...
		$this->patterns = array(
			...
			'/{img-thumb:(.+)\|(.+)}/U' => "<div class='thumb'><a href='$this->imgDir$1' title='Щракнете за увеличен размер'><img src='{$this->imgDir}thumb/$1' alt='$2' /></a><p><a href='$this->imgDir$1' title='Щракнете за увеличен размер'><img src='{IMGDIR}viewmag.png' /></a> $2</p></div>",
			'/\t/' => '      ', // six nbspaces
		);
	}
	...
	protected function wiki2html($s) {
		return preg_replace($this->kpatterns, $this->vpatterns, $s);
	}
}
?>
